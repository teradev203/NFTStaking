...
import { abi as nftTokenAbi } from "../abi/wsOHM.json";
import { abi as rewardTokenAbi } from "../abi/wsOHM.json";
import { abi as stakingAbi } from "../abi/wsOHM.json";
...
export const loadAccountDetails = createAsyncThunk(
  ...
  async ({ networkID, provider, address }: IBaseAddressAsyncThunk) => {
    ...
    const nftTokenContract = new ethers.Contract(
      addresses[networkID].NFT_TOKEN_ADDRESS as string,
      nftTokenAbi,
      provider,
    );

    const rewardTokenContract = new ethers.Contract(
      addresses[networkID].REWARD_TOKEN_ADDRESS as string,
      rewardTokenAbi,
      provider,
    );
